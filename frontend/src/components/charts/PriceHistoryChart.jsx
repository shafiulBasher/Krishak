...
const PriceHistoryChart = ({ data, cropName }) => {
    ...
    return (
      <div className="flex items-center justify-center h-64 bg-gray-50 rounded-lg">
        <div className="text-center">
          <p className="text-gray-500">{t('market.noPriceHistory')}</p>
          <p className="text-sm text-gray-400 mt-1">{t('market.priceWillAppear')}</p>
        </div>
      </div>
    );
  }
        ...
        <div className="bg-white p-3 shadow-lg rounded-lg border border-gray-200">
          ...
          <div className="mt-2 pt-2 border-t border-gray-100">
            <p className="text-xs text-gray-500">
              {t('market.margin')} ৳{(payload[1].value - payload[0].value).toFixed(0)}
            </p>
          </div>
        </div>
      );
  ...
  return (
    <div className="w-full">
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-gray-800">{cropName} - {t('market.priceTrend')}</h3>
        <p className="text-sm text-gray-500">{t('market.lastDays', { count: data.length })}</p>
      </div>
      ...
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
          ...
          <YAxis 
            tick={{ fontSize: 12 }}
            stroke="#9ca3af"
            label={{ value: t('market.priceKg'), angle: -90, position: 'insideLeft', style: { fontSize: 12 } }}
          />
      ...
      <div className="mt-4 grid grid-cols-2 gap-4">
        <div className="bg-blue-50 p-3 rounded-lg">
          <p className="text-xs text-blue-600 font-medium">{t('market.wholesaleRange')}</p>
          <p className="text-lg font-bold text-blue-700">
            {formatPrice(Math.min(...chartData.map(d => d.wholesale)))} - {formatPrice(Math.max(...chartData.map(d => d.wholesale)))}
          </p>
        </div>
        <div className="bg-green-50 p-3 rounded-lg">
          <p className="text-xs text-green-600 font-medium">{t('market.retailRange')}</p>
